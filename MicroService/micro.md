...
- 服务降级
    - 降级甚至关掉后台某些服务
- 特权请求
    - 在多租户下，考虑让一部分特殊用户有限处理。
- 延时处理
    - 利用队列把请求缓存住。               

## 限流实现方式
1、计数器                       
最简单的方式，维护一个计数器。来请求就加一，达到阈值直接拒绝请求。                                    
实践中一般使用ngnix+lua+redis，Redis中存计数。                                   

2、漏斗模式
流量就像进入到漏斗中的水，出口就和处理请求一样。                               
漏斗多是用队列实现，当流量过多时，队列积压，队列满了，则开始拒绝请求。                               

3、令牌桶
系统以恒定的速率产生令牌，然后把令牌放到令牌桶中，令牌桶中有容量，当令牌桶满了后，再往里放令牌就会被丢弃；
当想要处理一个请求时，需要从令牌桶中取出一个令牌，如果此时令牌桶中没有令牌，则拒绝该请求。                   
