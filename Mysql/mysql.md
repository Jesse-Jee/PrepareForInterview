...
MVCC的底层实现实际上本质上就是undo_log + 版本链 + read-view匹配规则。                    
当进行增删改时，会生成trx_id 和 roll_pointer。 roll_pointer指向上一个版本。                      
在可重复读隔离级别下，同一个事务的select会用在第一次select时产生的read-view.                       
read-view中包含：                   
所有未提交事务的ID数组和已创建的最大事务ID组成。              
如：                          
【100， 200】， 300                             
...
- 如果当前检查的ID小于min_id,可见。          
- 如果当前检查ID大于max_id,不可见。                          
- 如果当前检查的ID在min_id 和 max_id 区间内。
    - 看是否在未提交的数组中，如【100，200】中，如果在，不可见。但是，如果这个事务ID是自己的，那么可见。
    - 如果不在，可见。                        
...
把同一个表的数据按照一定规则拆分到不同的数据库中，每个库可以放在不同的服务器上。             
结果：
- 每个库结构一样
- 每个库数据不一样，没有交集
- 所有库的全集就是全量数据              
...
例如：商品基础信息和商品的详情页信息。在列表中只需要先显示商品基础信息如价格，名称，位置就可以了。
需要进一步加载的时候才需要查询详情页信息。                           

这样的好处是：
1、商品信息的浏览和查看详情的互不影响。                                          
2、商品信息操作的高效率不会被商品详情的低效率拖累。                                  


## 分库分表步骤
在系统设计阶段，就应该根据业务的耦合松紧程度来确定垂直分库，垂直分表。                         
在数据量和访问压力不是特别大的情况，首先考虑缓存、读写分离、索引技术等方案。                      
如果数据量极大，且持续增长，再考虑水平分库分表方案。                  
