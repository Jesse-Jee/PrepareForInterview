...
## TCP拥塞控制
对网络中某一资源的需求超过了该资源能提供的可用部分，网络性能就会变坏。这就是拥塞。  
四种拥塞控制方法：                      
- 慢开始
- 拥塞避免
- 快重传
...
- 发送方维护一个拥塞窗口cwnd的状态变量，其值取决于网络的拥塞程度并且动态变化。            
    - 拥塞窗口维护原则：只要网络没有出现拥塞，窗口就增大一点。只要出现拥塞，窗口就减小一点。                   
    - 判断拥塞的依据： 没有按时收到应当到达的确认报文。（发生重传）         

- 发送方把拥塞窗口作为发送窗口，即swnd = cwnd。
- 发送方维护一个慢开始门限ssthresh状态变量：
    - 当cwnd< ssthresh变量时。使用慢开始算法。拥塞窗口值可以指数增加
    - 当cwnd > ssthresh 变量时，停止使用慢开始，改为拥塞避免算法，拥塞窗口值改成每次只能加1。
        当发生拥塞时，ssthresh改为最大当前cwnd的一半，cwnd置为1。            
    - 当cwnd = ssthresh 变量时，既可以慢开始算法又可以拥塞避免算法。               

- 有时个别报文出现了丢失，但实际网络并没有发生拥塞。
    - 将导致发送方发生重传，并误以为发生了拥塞。
    - 然后会造成使用拥塞避免算法。
   
- 快重传就是在还没到发送超时前就重传。          
    - 这就要求，接收方不要等待自己发送数据时才捎带确认，而是要立即确认。             
    - 即使收到了失序的报文段也要立即发出对已收到的报文的重复确认。            
    - 发送方一旦收到3次连续的重复确认，就会将相应的报文段立即重传。               
    
- 发送方一旦收到3个重复确认，就知道只是丢失了个别报文，于是不启动慢开始算法，启动快恢复算法。
    - 发送方将慢开始门限的变量值和拥塞窗口值调整为当前窗口的一半，开始执行拥塞避免算法。
    - 也有的会认为丢失的只是丢失，没有拥塞，会把门限值加上丢失的个数。                  
